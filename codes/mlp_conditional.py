...
class MLP(nn.Module):
    def __init__(self, input_size, hidden_size, emb_dim=128, xs_dim=128):
        """
        Initializes the MLP model for processing the input and combined embeddings (conditional diffusion model).
        ...
        Args:
            ...
            xs_dim (int): The dimensionality of the static attribute embedding. Default is 128.
        """
        ...
        self.layers = nn.ModuleList([
            ...
            nn.GELU(),
            nn.Linear(hidden_size + hidden_size + emb_dim + xs_dim, hidden_size),
            nn.GELU(),
            nn.Linear(hidden_size + hidden_size + emb_dim + xs_dim, hidden_size),
            nn.GELU(),
            nn.Linear(hidden_size + hidden_size + emb_dim + xs_dim, hidden_size)
